"""V1 analysis runs and feedback."""

import sqlalchemy as sa
from alembic import op
from sqlalchemy.dialects.postgresql import JSONB, UUID

revision = "0001_product_telemetry"
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    bind = op.get_bind()
    analysis_run.create(bind, checkfirst=False)
    feedback.create(bind, checkfirst=False)


def downgrade() -> None:
    bind = op.get_bind()
    feedback.drop(bind, checkfirst=False)
    analysis_run.drop(bind, checkfirst=False)
